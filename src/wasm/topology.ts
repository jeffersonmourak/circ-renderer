 ...
 * Two versions are accepted; the only difference is the per-component body:
 *
 *   magic[4]="CIRF", version u8 (0x01 or 0x02),
...
export function decodeFullTopology(bytes: Uint8Array): FullTopology {
  ...
  for (let i = 0; i < numComponents; i++) {
    ...
    // v02 kind-dispatched aux suffix: only `slice` carries trailing (lo, hi).
