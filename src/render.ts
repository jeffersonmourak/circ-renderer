...
import { loadCircFile } from "./services";
...
export function buildCanvas(
  ...
  const loader = loadCircFile(content);

  const circuit = parseCircuit(loader);
  ...
  function processPointerAction(collidingComponents: Set<number>) {
    for (const index of collidingComponents) {
      ...
      const newSignals = component.onPress?.(
        simulation.portsSignals[index],
        component.state
      );
  ...
  createCanvasController(canvasElement, {
    ...
    draw: ({ canvas, context }) => {
      ...
      for (let i = 0; i < components.length; i++) {
        ...
        const angle = component.faceAngles[toFaceIndex(component.facing)] + 90;
