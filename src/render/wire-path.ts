...
/**
 * Trace a wire into `path`, one subpath per segment.
 *
 ...
 * horizontal one as the wire that arcs.
 */
export function traceWire(path: CanvasPath, wire: RoutedWire, cell: number, arcRadius: number = defaultArcRadius(cell)): void {
 ...
 */
export function wirePath(wire: RoutedWire, cell: number, arcRadius: number = defaultArcRadius(cell)): Path2D {
  const path = new Path2D();
  traceWire(path, wire, cell, arcRadius);
  return path;
}
